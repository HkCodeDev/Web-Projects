...
function toCamelCase(name, uppercase = false) {
  ...
  return uppercase ? result : result[0].toLowerCase() + result.slice(1);
}
/*******************************************************************************
...
function parseDateString(value) {
  ...
  if (value + '' !== value) {
    throw { message: errorMessage, toString: () => errorMessage };
  }

  const ymdRegex = /^(\d{4})\/(\d{2})\/(\d{2})$/;
  const dmyRegex = /^(\d{2})\/(\d{2})\/(\d{4})$/;

  const ymdMatch = value.match(ymdRegex);
...
function toDateString(value, format) {
  try {
    if (!value || !value.getMonth || !value.getFullYear || !value.getDate) {
      throw 'Invalid input: Expected a valid Date object';
    }

    const dateCheck = new Date(value.getTime());
    ...
    switch (format) {
      ...
      default:
        throw 'Invalid date format. Use "YYYY/MM/DD", "DD/MM/YYYY", or "MM/DD/YYYY"';
    }
  } catch (error) {
    throw `Error formatting date: ${error}`;
  }
}
...
function normalizeTime(value) {
  const amPmRegex = /^(\d{1,2}):(\d{2}):(\d{2}) (AM|PM)$/i;
  const twentyFourHourRegex = /^(\d{1,2}):(\d{2}):(\d{2})$/;

  const amPmMatch = value.match(amPmRegex);
  if (amPmMatch) {
    ...
    const twentyFourHourMatch = value.match(twentyFourHourRegex);
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  if (!resourceType || !resourceId) {
    throw 'Invalid arguments. Both resourceType and resourceId must be provided.';
  }
  ...
  if (extraData !== undefined) {
    if (!extraData || extraData.trim() === '') {
      throw 'Invalid extraData. It must be a non-empty string.';
    }
