...
function getTotalResults(data) {
  // Return the `total_results` property from the data object
...
function speciesCoordinates(data) {
  // Iterate over the results array using a for-loop
  for (let i = 0; i < data.results.length; i++) {
    let observation = data.results[i];
    // Extract species_guess and location, and log them in the specified format
...
function speciesCoordinates2(data) {
  // Use forEach to iterate over the results array
...
function speciesCoordinates3(data) {
  // Use for-of loop to iterate over the results array
  for (const observation of data.results) {
    // Format the location with a space after the comma
    const formattedLocation = observation.location.replace(',', ', ');
    // Extract species_guess and log the observation in the specified format
...
function observationsByQualityGrade(data, qualityGrade) {
  // Convert qualityGrade to lowercase if it's a string
  ...
  // Check if qualityGrade is valid
  if (!validGrades.includes(qualityGrade)) {
    throw new Error('Invalid qualityGrade value');
  }

  // Filter and return observations with the matching quality grade
  return data.results.filter((observation) => observation.quality_grade === qualityGrade);
}
...
function observationsByQualityGrades(data, ...qualityGrades) {
  // Throw an error if no quality grades are provided
  if (qualityGrades.length < 1) {
    throw new Error('At least one qualityGrade must be provided');
  }

  // Initialize an array to hold all matching observations
  let allObservations = [];

  // Iterate over each qualityGrade, and concatenate the results
  for (const qualityGrade of qualityGrades) {
    // Reuse the observationsByQualityGrade function for each qualityGrade
...
function getObservationsByTaxa(data, ...taxaNames) {
  const knownTaxa = ['Animalia', 'Aves', 'Insecta', 'Plantae'];
  let filteredObservations = [];

  taxaNames.forEach((taxon) => {
    if (knownTaxa.includes(taxon)) {
      const observationsForTaxon = data.results.filter(
        (observation) => observation.taxon.iconic_taxon_name === taxon
      );
      filteredObservations = filteredObservations.concat(observationsForTaxon);
    }
  });
...
function getObservationsByLocation(data, options = {}) {
  const filterByExactLocation = (observation, lat, lng) => {
    const [obsLat, obsLng] = observation.location.split(',').map(Number);
    return obsLat === lat && obsLng === lng;
  };

  const filterByRange = (observation, latRange, lngRange) => {
    const [obsLat, obsLng] = observation.location.split(',').map(Number);
    return (
      obsLat >= latRange.min &&
      obsLat <= latRange.max &&
  ...
  return data.results.filter((observation) => {
    if (
      ...
      return filterByExactLocation(observation, options.lat, options.lng);
    } else if (
      options.lat &&
      options.lng &&
      typeof options.lat === 'object' &&
      typeof options.lng === 'object'
    ) {
      return filterByRange(observation, options.lat, options.lng);
    } else {
      return true;
    }
  });
