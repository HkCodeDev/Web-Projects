...
const MONGODB_URI = 'mongodb+srv://hkarimi9:Hk137913@cluster.jyodg8j.mongodb.net/?retryWrites=true&w=majority&appName=Cluster';

app.use(clientSessions({
    cookieName: 'session',  // Cookie name to access session object in templates
    secret: 'MN123hobsIBB89rwyffit889HHJk', // A secret key for encrypting session data
...
app.use(function(req, res, next) {
    res.locals.session = req.session; // Make session available to templates
    next();
});
function ensureLogin(req, res, next) {
    if (!req.session.userName) { // Check if the user is not logged in
...
app.get('/items', (req, res) => {
    console.log('Accessing items route');
    storeService.getAllItems()
        ...
        .catch((err) => {
            console.error('Error accessing items:', err);
            res.render('items', { message: "no results" });
        });
});
...
app.get('/logout', (req, res) => {
    req.session.reset(); // Reset the session to log the user out
    res.redirect('/');
});
...
storeService.initialize()
    .then(() => authData.initialize(MONGODB_URI))  // Adding authData initialization
    .then(() => {
        app.listen(port, () => {
    ...
    .catch((err) => {
        console.log('Failed to initialize the services:', err);  // Generic error message for any failure
    });
